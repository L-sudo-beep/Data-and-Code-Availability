...
# --- 新增参数，用于计算特定平面的rRMSE ---
PLANE_Y_FOR_RRMSE = 1.26  # <--- 新增: 指定计算rRMSE的Y平面值
...
model.eval()  # <--- 修改: 明确设置为评估模式，这会禁用Dropout和BatchNorm的更新
...
plt.figure(figsize=(12, 7))  # 稍微调大画布
...
plt.plot(df_metrics["Case"], df_metrics[f"rRMSE_plane_Y={closest_y_val:.2f}m"], '-x', color='purple',
         label=f"rRMSE [Plane Y={closest_y_val:.2f}m]")  # 新增曲线
