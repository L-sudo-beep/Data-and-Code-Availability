...
def visualize_results(true_temp, pred_temp, coords, case_index, save_dir):
    """
    绘制当前测试工况在指定截面上的真实温度场、重构温度场和绝对误差云图。
    误差云图标题中显示当前截面最大绝对误差。
    """
    ...
    # 当前默认绘制 y=constant 截面，因此横轴为 X，纵轴为 Z
    if PLANE_TYPE == 'y':
        x_plot = points_to_interp[:, 0]
        z_plot = points_to_interp[:, 2]
        x_label = "X (m)"
        z_label = "Z (m)"
    elif PLANE_TYPE == 'x':
    ...
    grid_z = np.linspace(z_plot.min(), z_plot.max(), 150)
    GX, GZ = np.meshgrid(grid_x, grid_z)

    Tt = griddata((x_plot, z_plot), t_true, (GX, GZ), method='cubic')
    Tp = griddata((x_plot, z_plot), t_pred, (GX, GZ), method='cubic')
    err = np.abs(Tt - Tp)

    # 当前工况在该截面上的最大绝对误差
    err_max_current = float(np.nanmax(err))
    ...
    out_path = os.path.join(
        save_dir,
        f"case_{case_index}_reconstruction.png"
    )
...
def build_ordered_grid_indices(Nx, Ny, Nz, order):
    ...
    order:
        'yzx'：C-order flatten 时 X 方向变化最快；
        'xzy'：C-order flatten 时 Y 方向变化最快；
        'xyz'：C-order flatten 时 Z 方向变化最快。
    """
...
print("Saved current vectorization adjacency stats to:")
...
# 保存全局最大误差，供 Tucker / CP 统一色标使用
np.save(
    os.path.join(SAVE_DIR, "global_error_max.npy"),
    global_error_max
)
...
print("-" * 40)
print(f"Global max error on plotted slices  : {global_error_max:.4f} °C")
...
print("Saved per-case metrics to:")
...
print("Saved summary metrics with adjacency stats to:")
print(summary_path)
