...
# ======================================
# 设置随机种子以保证结果可复现 (Set Seeds for Reproducibility)
# ======================================
random.seed(SEED)
np.random.seed(SEED)
...
# ======================================
# 验证与误差计算 (Validation and Error Calculation)
# ======================================
print("\n在测试集上进行验证并计算误差...")
model.eval()
...
for i, idx in enumerate(tqdm(val_idx, desc="验证案例")):
    ...
    # =================================================================================
    #  关键点: 下面这行代码将【每个测试工况】的rRMSE和点占比等指标汇总到一起输出
    # =================================================================================
...
# ======================================================================
# 定义10个预选高温点 (Define 10 pre-selected high-temp points)
# ======================================================================
...
for point_idx in pre_selected_indices:
    ...
    # 从之前计算好的大矩阵中，抽取出这个点在所有验证案例中的温度序列
...
# ======================================================================
# 计算10个预选高温点的rRMSE_n (Calculate rRMSE_n for high-temp points)
# ======================================================================
...
for point_idx in pre_selected_indices:
    if point_idx >= N_points:
        # 这个警告在上面已经有了，这里可以省略或保留
        continue

    # all_true_val_temps 和 all_pred_val_temps 的维度是 (N_points, num_validation_cases)
    # 我们需要提取特定点在所有验证案例中的温度序列
    true_series = all_true_val_temps[point_idx, :]
    pred_series = all_pred_val_temps[point_idx, :]
