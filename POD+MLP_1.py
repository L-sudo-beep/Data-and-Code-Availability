...
# --- 可视化与误差计算参数 ---
PLANE_TYPE = 'y'
PLANE_VALUE = 1.26  # 用于可视化的Y平面值
NUM_ADJACENT_PLANES = 0
PLANE_Y_FOR_RRMSE = 2.52  # <--- 新增: 指定计算rRMSE的Y平面值
...
# ======================================
# Helper Functions (MLP, Visualization)
# ======================================
class MLP(nn.Module):
...
# ======================================
allowed_orders = ['xzy', 'yxz', 'zxy']
chosen_order = ""
while chosen_order not in allowed_orders:
    prompt_message = f"请选择数据展开方式 {tuple(allowed_orders)}: "
    chosen_order = input(prompt_message).lower().strip()
...
if chosen_order == 'xzy':
    permuted_tensor = np.transpose(all_snapshots_tensor, (0, 2, 1, 3))
    coords = sorted_coords[np.lexsort((sorted_coords[:, 1], sorted_coords[:, 2], sorted_coords[:, 0]))]
...
# --- 新增: 在循环外预先计算平面掩码，提高效率 ---
unique_y_coords_in_current_order = np.unique(coords[:, 1])
# 找到与目标值最接近的Y坐标
closest_y_val = unique_y_coords_in_current_order[
    np.argmin(np.abs(unique_y_coords_in_current_order - PLANE_Y_FOR_RRMSE))]
# 创建布尔掩码，用于筛选该平面的所有点
plane_mask_for_rrmse = (coords[:, 1] == closest_y_val)
num_plane_points = np.sum(plane_mask_for_rrmse)

print(f"\nTarget plane for rRMSE calculation: Y = {PLANE_Y_FOR_RRMSE:.4f} m")
if num_plane_points > 0:
    print(f"Found {num_plane_points} points on the closest available plane: Y = {closest_y_val:.4f} m")
else:
...
for i, original_case_idx in enumerate(tqdm(val_idx, desc="Validating and Visualizing Cases")):
    ...
    # 2. 特定平面的rRMSE计算 <--- 新增逻辑
    rRMSE_case_plane = np.nan  # 默认为nan，如果无法计算
    if num_plane_points > 0:
        # 使用预先计算好的掩码来提取平面数据
        true_temp_plane = true_temp[plane_mask_for_rrmse]
        rec_temp_plane = rec_temp[plane_mask_for_rrmse]
    ...
    # <--- 修改: 打印信息中加入平面rRMSE ---
    print(
        f"  Case {human_case_number:<2d}: MAE={mae:.4f}, RMSE={rmse:.4f}, rRMSE_Full={rRMSE:.6f}, rRMSE_Plane(Y={closest_y_val:.2f})={rRMSE_case_plane:.6f}")
...
print(f"  Mean rRMSE (Full)   : {np.mean(rRMSE_list):.6f}")
print(f"  Mean rRMSE (Plane)  : {np.nanmean(rRMSE_plane_list):.6f}")  # <--- 修改: 汇总平面rRMSE
print("-" * 40)
...
# <--- 修改: 保存每个案例的详细指标到CSV，增加平面rRMSE列 ---
df_metrics = pd.DataFrame({
    "Case": [i + 1 for i in val_idx],
    "MAE": mae_list,
    "RMSE": rmse_list,
    "rRMSE_Full": rRMSE_list,
    f"rRMSE_Plane_Y={closest_y_val:.2f}": rRMSE_plane_list
})
