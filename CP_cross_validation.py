...
# -*- coding: utf-8 -*-
"""
CP/PARAFAC Rank Selection by Internal Cross-Validation
+ Final Independent-Test Evaluation
+ MLP Regression, DCR and Computational-Time Benchmarking
==========================================================================
...
2. 固定独立测试工况：9 组
   ...
   - 不参与 CP 秩选择；
   - 只在秩确定后，用全部 80 组重新训练最终模型；
   - 最终在这 9 组上评价 CP 模型，并用于与 Tucker/POD 比较。

选秩规则
--------
默认使用 one-standard-error rule（一倍标准误差规则）：

    R_min = argmin_R mean_CV_rRMSE(R)

    threshold = mean_CV_rRMSE(R_min) + SE_CV_rRMSE(R_min)

    R_selected = 满足 mean_CV_rRMSE(R) <= threshold 的最小秩

这样选择的是“与最低交叉验证误差统计上相近的最小秩”，
可避免仅因极小的误差改善而选取过高 CP 秩。
...
RUN_FINAL_TEST_RANK_SWEEP = False
    False：独立测试集只评价交叉验证选出的 CP 秩（推荐）。
    True ：仍在 9 组测试集上运行全部候选秩，仅用于敏感性展示；
           但最终秩仍由 80 组内部交叉验证确定。

CV_CP_N_STARTS / FINAL_CP_N_STARTS
    CP 多初始化次数。
    默认 1，节省时间。
    若设为 >1，建议同时将对应 INIT 设为 "random"。
    程序会用训练折上的 CP 空间基投影误差选择最佳初始化。
...
说明
----
CP 分量一般不正交，因此不能像 POD/Tucker-HOSVD 一样严格依据累计能量选秩。
本程序通过“未见工况端到端重构误差”选取 CP 秩，能够作为论文中的主要选秩依据。
"""
...
# 最终模型 CP 初始化设置
FINAL_CP_N_STARTS = 1
FINAL_CP_INIT = "svd"       # 多启动时建议设为 "random"
...
# 为保证选秩与最终模型训练口径一致，默认与正式训练轮数相同。
# 若首次调试计算量过大，可临时调低，但正式论文结果建议恢复一致。
CV_MLP_EPOCHS = 2000

CV_CP_N_STARTS = 1
CV_CP_INIT = "svd"          # 多启动时建议设为 "random"

# 若不是 None，则最终强制使用该秩；交叉验证仍可运行并保存结果。
FORCE_CP_RANK = None

# 推荐 False：测试集只评价交叉验证选出的秩。
RUN_FINAL_TEST_RANK_SWEEP = False
...
def to_jsonable(value):
    """递归转换 NumPy/Pandas 类型，避免 json.dump 序列化失败。"""
...
def calculate_rrmse_n_hot_points(
    ...
    """
    沿用原程序定义：从训练集平均温度场中选取温度最高的若干个
    “不同温度值对应的首个网格点”，计算这些点跨独立测试工况的 rRMSE_n。
    """
...
def cp_build_projection_matrix(
    ...
    """
    M[:, r] = lambda_r * vec(a_r outer b_r outer c_r)
    """
    nx, rank_r = Ax.shape
    ny = Ay.shape[0]
    nz = Az.shape[0]
...
def project_tensor_cases_to_cp_coeff(
    ...
    """
    一次性投影多个工况，避免对每个工况重复分解同一个最小二乘矩阵。
    ...
    projection_rRMSE:
        所有训练折中心化场在 CP 空间基上的整体相对投影误差。
        多初始化时用它选择最稳定/拟合最好的 CP 空间基。
    """
...
def fit_cp_basis_and_project_train(
    ...
    最优初始化依据：
        训练折中心化温度场投影到 CP 空间基后的整体 rRMSE 最小。
    """
...
# ======================================================================
# Regression Preparation / Output Decoding
# ======================================================================
...
# ======================================================================
# Internal Cross-Validation for CP Rank Selection
# ======================================================================
...
def run_internal_cp_rank_cross_validation(
    ...
    for rank_position, cp_rank in enumerate(rank_list, start=1):
        ...
        for fold_index, (fold_train_local, fold_val_local) in enumerate(
            ...
            # ----------------------------------------------------------
            # 1. Fold-specific mean and centering
            # ----------------------------------------------------------
            if CENTER_ALONG_N:
            ...
            # ----------------------------------------------------------
            # 2. Fold-specific CP decomposition and coefficient projection
            # ----------------------------------------------------------
            cp_seed = (
                CV_RANDOM_STATE
                + cp_rank * 1000
            ...
            # ----------------------------------------------------------
            # 3. Fold-specific regression model
            # ----------------------------------------------------------
            ...
            # ----------------------------------------------------------
            # 4. Reconstruct each validation case and compute metrics
            # ----------------------------------------------------------
            fold_rrmse_values = []
            fold_mae_values = []
            fold_rmse_values = []
...
# ======================================================================
# Final One-Rank Experiment on Independent Test Set
# ======================================================================
...
def run_one_rank_final_test(
    ...
    # --------------------------------------------------------------
    # CP decomposition on all 80 development cases
    # --------------------------------------------------------------
    decomposition_start = time.perf_counter()
    ...
    # --------------------------------------------------------------
    # Reconstruction and independent-test metrics
    # --------------------------------------------------------------
    predicted_fields = []
    ground_truth_fields = []
    metric_rows = []
...
def main():
    ...
    # --------------------------------------------------------------
    # Internal CV: only 80 development cases
    # --------------------------------------------------------------
    cv_result = None
