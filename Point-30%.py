...
新增功能：
1. 统计左右两列机架出风口区域内的候选网格点总数；
2. 使用训练集平均温度场，对候选点按温度降序排序；
3. 选取温度最高的前 30% 网格点；
4. 保存所选点的索引、区域、坐标、训练集平均温度、rRMSE_n，
   以及各测试工况的真实温度、预测温度和绝对误差。

左列机架出风口：
X=1.69–1.90 m，Y=0.67–2.77 m，Z=3.06–6.03 m

右列机架出风口：
X=4.65–4.95 m，Y=0.67–2.77 m，Z=3.06–6.03 m
"""
...
def interval_mask(
    ...
    """Inclusive interval mask with numerical tolerance."""
    return (
...
def build_outlet_region_masks(
    ...
    """
    Build 3-D Boolean masks for the left and right rack-outlet regions.
    """
    left_mask = (
...
def select_top_outlet_hotspots(
    ...
    """
    Select the hottest top fraction of points in both outlet regions.

    The ranking is based on the training-set mean temperature field.
    The selected count is ceil(total candidate count * top_fraction).
    """
...
def append_hotspot_prediction_metrics(
    ...
    """
    Add rRMSE_n and each test case's true, predicted and absolute-error values.
    """
    detailed_rows = []
...
def main():
    ...
    # Add rRMSE_n and all test-case temperature/error information.
