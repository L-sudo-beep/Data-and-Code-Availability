...
# CP 分解设置（取代 Tucker）
CP_RANK        = 16        # 建议网格搜索 {6,8,10,12,14,16,20} 用验证误差选择
CP_N_ITER_MAX  = 2000
CP_TOL         = 1e-7
...
def cp_reconstruct_from_d(d: np.ndarray, Ax, Ay, Az, lambdas, T_mean_train=None):
    ...
    """
    NX, R = Ax.shape; NY = Ay.shape[0]; NZ = Az.shape[0]
...
def main():
    ...
    if USE_PCA_BOTTLENECK:
        print(f"=== PCA bottleneck on CP coeffs (q={PCA_LATENT_Q}) ===")
    ...
    plt.xlabel("Case")
    plt.ylabel("Error (°C)")
    plt.title("CP+MLP Per-case Error (TRAIN-only)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(FIG_DIR, "mae_rmse_compare_cp.png"), dpi=200)
    plt.close()
    ...
    for k, case_idx in enumerate(idx_test):
        ...
        plt.suptitle(f"Y={ys[y_slice_index]:.3f} m | Unified Error Colormap (CP)")
        ...
        ax2.set_title("Reconstructed Temperature (CP)"); ax2.set_xlabel("X (m)"); ax2.set_ylabel("Z (m)")
