...
# —— 旧的 MLP 超参将不再使用，可保留以便对照 ——
HIDDEN_LAYERS = [128, 256, 128]

# —— 新的 Transformer 回归头超参 ——
D_MODEL = 128          # token embedding dim
NHEAD = 4              # multi-head attention heads
NUM_LAYERS = 3         # encoder layers
DIM_FF = 256           # feedforward dim in encoder layer
DROPOUT = 0.1          # dropout rate

LEARNING_RATE = 1e-3   # 可保持不变
EPOCHS = 2000
BATCH_SIZE = 16
...
SAVE_DIR = r'C:\Users\Lenovo\Desktop\POD89_Fixed_Test_Set1'
...
# 【固定测试集】一基：8,18,28,38,48,58,68,78,88 → 零基
...
# ======================================
# Transformer Regressor (替换原 MLP)
# ======================================
class TransformerRegressor(nn.Module):
    """
    将 14 维条件看作长度为 14 的序列：
      - 对每个标量特征做值嵌入 (Linear 1->d_model)
      - 加上特征类型嵌入 (Embedding(14, d_model))
      - 经过 TransformerEncoder
      - 序列均值池化 -> 线性映射到 K 维 POD 系数
    """
    def __init__(self, num_features: int, output_size: int,
        ...
        self.value_embed = nn.Linear(1, d_model)               # 对每个标量值的投影
        self.type_embed = nn.Embedding(num_features, d_model)  # 每个特征一个可学习类型向量
    ...
    def forward(self, x):
        """
        x: (batch, num_features) = (B, 14)
        """
        B = x.size(0)
        # (B, 14, 1) -> (B, 14, d_model)
        v = self.value_embed(x.unsqueeze(-1))
        # (B, 14)
        idx = torch.arange(self.num_features, device=x.device).unsqueeze(0).expand(B, -1)
        t = self.type_embed(idx)  # (B, 14, d_model)

        z = v + t                 # (B, 14, d_model)
        z = self.encoder(z)       # (B, 14, d_model)
        z = z.mean(dim=1)         # 均值池化 (B, d_model)
        out = self.head(z)        # (B, K)
        return out

# 实例化 Transformer 模型
model = TransformerRegressor(
    num_features=conditions_train_norm.shape[1],
    output_size=K,
    d_model=D_MODEL,
    nhead=NHEAD,
