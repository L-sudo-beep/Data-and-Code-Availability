...
# -*- coding: utf-8 -*-
"""
Tucker + Transformer — Residual Alpha + Energy-Weighted + Sampled Field Loss
-------------------------------------------------------------------------------
- 仅用训练集做 HOSVD（无泄漏）
- 学习目标：delta = alpha - alpha_mu（抗均值坍缩）
- 额外场空间监督：随机体素点上直接监督温度值（Huber），避免只把系数推向均值
- 系数损失使用能量加权（||A[:,k]||_2）MSE
- 课程学习：逐步提高场监督权重 λ_field
- 与 POD+Transformer 相同的配色/色标逻辑，便于误差图横向比较
...
CENTER_ALONG_N = True
SCALE_INPUTS = True
SCALE_OUTPUT_COEFF = True      # 对 delta 而非 alpha

ALPHA_RIDGE_LAMBDA = 1e-6      # 释放方差，抗收缩
...
# Sampled field loss
VOXELS_PER_ITEM = 2000          # 每个样本每步随机抽取体素点数
LAMBDA_COEF = 0.4               # 系数损失初始权重
LAMBDA_FIELD_START = 0.2        # 场损失初始权重
LAMBDA_FIELD_END   = 0.8        # 课程学习最终权重
HUBER_DELTA = 1.0               # Huber 损失阈值
...
def ridge_pinv(A: np.ndarray, lam: float):
    AtA = A.T @ A
    r = AtA.shape[0]
    return np.linalg.solve(AtA + lam*np.eye(r), A.T)  # (rn, m)
...
# ---------- Sampled field supervision ----------

def make_B4(A: np.ndarray, rx: int, ry: int, rz: int) -> np.ndarray:
    """Reshape A (rx*ry*rz, rn) -> (rx, ry, rz, rn)"""
    rn = A.shape[1]
...
def field_rows_for_voxels(Ux_t: torch.Tensor, Uy_t: torch.Tensor, Uz_t: torch.Tensor,
    ...
    """
    高效计算 L_rows: 对给定体素 (ix,iy,iz)，返回每个点对应的 (rn,) 行向量
    L_row(ix,iy,iz) = (Ux[ix,:] ⊗ Uy[iy,:] ⊗ Uz[iz,:]) @ A
                    == einsum 连乘：(((Ux[ix] · B4) over x) · over y) · over z
    返回: (S, rn)
    """
    ...
    uy = Uy_t[iy]                        # (S, ry)
    uz = Uz_t[iz]                        # (S, rz)
    # step1: t = einsum('sr, r y z n -> s y z n', ux, B4)
    t = torch.einsum('sr,ryzn->syzn', ux, B4_t)
    # step2: t2 = einsum('sy, s y z n -> s z n', uy, t)
    t2 = torch.einsum('sy,syzn->szn', uy, t)
    # step3: rows = einsum('sz, s z n -> s n', uz, t2)
...
def build_random_voxels(n_sample: int, nx: int, ny: int, nz: int, device: str):
    ...
    return torch.stack([ix, iy, iz], dim=1)   # (S, 3)
...
def main():
    ...
    # prepare torch constants for field loss (on device)
    ...
    # numpy views for T_train_used -> torch (延后到用到时再搬到 device，避免大显存占用)
    T_train_np = T_train_used  # (NX,NY,NZ,Ntr)
    ...
    for rn_sel in rn_grid:
        ...
        # 上行确保 slice 安全；也可直接 G_train[..., :rn_sel]

        A = build_A(G_trunc)                                # (rx*ry*rz, rn_sel)
        M = ridge_pinv(A, ALPHA_RIDGE_LAMBDA)               # (rn_sel, rx*ry*rz)
        ew = np.linalg.norm(A, axis=0) + 1e-12              # energy weights
        B4 = make_B4(A, rx, ry, rz)                         # (rx,ry,rz,rn_sel)
        ...
        # compute alphas (train_sub/val_sub)
        ...
        # ground-truth fields (train/val) on device按需抽样，不整块搬运
        def fetch_gt_samples(local_indices, S):
            """
            返回：values (sum_S,), idx_xyz (sum_S,3), case_offsets (len(local_indices)+1,)
            便于分割还原每个样本的 S 个值
            """
        ...
        for ep in range(1, EPOCHS+1):
            ...
            for i in range(0, len(order), BATCH_SIZE):
                ...
                opt.zero_grad()
                # 系数残差预测
                d_pred = model(xb)                          # (B, rn)
                # 能量加权系数损失
                loss_coef = weighted_mse(d_pred, yb, model.ew)

                # 场监督：随机抽取每个样本 S 个体素
                with torch.no_grad():
                    gt_vals, idx_xyz, offsets = fetch_gt_samples(sel, VOXELS_PER_ITEM)  # (B*S,), (B*S,3)
                # 将残差还原为 alpha
                if SCALE_OUTPUT_COEFF:
                    # 反标准化（手动实现）：y = (d_pred * sigma) + mu；这里 sigma/mu 由 y_scaler_sel 得到
                    ...
                    d_denorm = d_pred
                alpha_pred = d_denorm + alpha_mu_t          # (B, rn)

                # 对每个样本，计算 S 个体素的线性行向量并与 alpha 相乘 -> 预测温度
                preds_field = torch.empty_like(gt_vals)
                for b in range(len(sel)):
                    rows = field_rows_for_voxels(Ux_t, Uy_t, Uz_t, B4_t,
                                                 idx_xyz[offsets[b]:offsets[b+1]])  # (S,rn)
                    preds_field[offsets[b]:offsets[b+1]] = rows @ alpha_pred[b]      # (S,)
    ...
    best_val = float("inf"); best_state=None; last_improve=0
    T_train_np = T_train_used  # for sampling
    ...
    # 可视化：与 POD+Transformer 相同的配色/色标
