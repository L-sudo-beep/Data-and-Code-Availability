...
def build_ordered_grid_indices(Nx, Ny, Nz, order):
    ...
    ----
    Nx, Ny, Nz : int
        三维规则网格在 x, y, z 方向上的节点数。

    order : str
        向量化展开顺序，例如：
        'xyz'：张量轴顺序为 (x, y, z)，C-order flatten 时 z 方向变化最快；
        'yxz'：张量轴顺序为 (y, x, z)，C-order flatten 时 z 方向变化最快；
        'zxy'：张量轴顺序为 (z, x, y)，C-order flatten 时 y 方向变化最快。

    返回
    ----
    index_sequence : ndarray, shape = (Nx*Ny*Nz, 3)
        一维向量中每个位置对应的三维网格索引 [ix, iy, iz]。
    """
...
def compute_vector_adjacency_stats(index_sequence, Nx, Ny, Nz, order_name):
    ...
    一维向量中的相邻元素对为：
        (index_sequence[0], index_sequence[1]),
        (index_sequence[1], index_sequence[2]),
        ...
        (index_sequence[N-2], index_sequence[N-1])

    若两个点在原三维网格中只相差一个 x 索引，则认为是 x 方向真实邻接；
    若只相差一个 y 索引，则认为是 y 方向真实邻接；
    若只相差一个 z 索引，则认为是 z 方向真实邻接；
    否则认为不是三维真实邻接。
    """
