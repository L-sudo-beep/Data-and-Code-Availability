...
# -*- coding: utf-8 -*-
"""
CP/PARAFAC Rank Sweep + MLP with DCR and Computational-Time Benchmarking
------------------------------------------------------------------------
一次实验中依次运行多个 CP 秩：
...
# 根目录。每个秩会自动保存到 FIG_ROOT_DIR/rank_06, rank_08, ...
FIG_ROOT_DIR = r"C:\Users\Lenovo\Desktop\TensorPOD89\figures_improved_CP_rank_sweep"
...
# 为保证 POD、Tucker、CP 的时间对比公平，三个程序应使用相同设置。
...
def predict_full_field_online(
    ...
    scaled_output = predict_scaled_output_one_case(
        ...
        output_scaler=None,
    ) if False else predict_scaled_output_one_case(
        model=model,
        raw_condition=raw_condition,
        input_scaler=input_scaler,
        scale_inputs=scale_inputs,
...
def save_error_metrics_plot(
    ...
    plt.title(
        f"CP+MLP Per-case Error Metrics "
        f"(TRAIN-only, R={cp_rank})"
    )
    plt.legend()
    plt.grid(
        True,
...
def save_rank_sweep_summary_outputs(
        summary_df: pd.DataFrame,
        fig_root_dir: str,
):
    summary_path = os.path.join(
        fig_root_dir,
        "cp_rank_sweep_summary.csv",
    )
    ...
    plt.xlabel("CP Rank R")
    plt.ylabel("Error Value")
    plt.title("Accuracy Metrics vs CP Rank")
    plt.grid(True, which="both", linestyle="--")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("CP Rank R")
    plt.ylabel("Time (s)")
    plt.title("Offline Computational Time vs CP Rank")
    plt.grid(True, which="both", linestyle="--")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("CP Rank R")
    plt.ylabel("Time (ms)")
    plt.title("Online Computational Time vs CP Rank")
    plt.grid(True, which="both", linestyle="--")
    plt.legend()
    plt.tight_layout()
...
def run_one_rank(
    ...
    print("\n" + "#" * 72)
    print(f"### Start CP/PARAFAC experiment for rank R = {cp_rank}")
    print("#" * 72)
    ...
    if CENTER_ALONG_N:
        ...
        print(
            "Applied N-mode mean centering using TRAIN-set mean."
        )
    else:
        T_mean_train = None
    ...
    for test_position, case_index in enumerate(idx_test):
        ...
        ground_truth_fields.append(T[..., case_index])
    ...
    for temperature_value in reversed(hottest_unique_temperatures):
        ...
        if flat_index not in hottest_point_indices:
            hottest_point_indices.append(int(flat_index))
    ...
    cp_weight_preview = cp_component_energy_weights(
        lambdas,
        Ax,
        Ay,
        Az,
    ...
    summary = {
        ...
        "output_paths": {
            ...
            "metrics_summary_cp_json": os.path.abspath(
                os.path.join(
                    fig_dir,
    ...
    print(
        "\n"
        + "=" * 20
        + f" FINAL SUMMARY | R={cp_rank} "
        + "=" * 20
    )

    print(
        f"Rank execution time: "
        f"{rank_total_time_seconds:.2f} s"
    )
    ...
    rank_summary_row = {
        ...
        "dnn_inference_mean_ms": float(
            timing_results["dnn_inference_time_per_case"]["mean_ms"]
        ),
        "dnn_inference_std_ms": float(
            timing_results["dnn_inference_time_per_case"]["std_ms"]
        ),
        "field_reconstruction_mean_ms": float(
            timing_results["field_reconstruction_time_per_case"]["mean_ms"]
        ),
        "field_reconstruction_std_ms": float(
            timing_results["field_reconstruction_time_per_case"]["std_ms"]
        ),
        "total_online_prediction_mean_ms": float(
            timing_results["total_online_prediction_time_per_case"]["mean_ms"]
        ),
        "total_online_prediction_std_ms": float(
            timing_results["total_online_prediction_time_per_case"]["std_ms"]
        ),
        ...
        "compression_percent_train_cp": float(
            current_dcr_row["compression_percent_train_cp"]
        ),
...
def main():
    ...
    print("=== Calculate CP DCR for all target ranks ===")
    ...
    print(
        cp_dcr_df[dcr_print_columns].to_string(index=False)
    )
    ...
    all_rank_summary_rows = []

    for cp_rank in CP_RANK_LIST:
        rank_summary_row = run_one_rank(
            cp_rank=cp_rank,
            X_params=X_params,
            T=T,
            xs=xs,
        ...
        all_rank_summary_rows.append(rank_summary_row)

    rank_summary_df = pd.DataFrame(
        all_rank_summary_rows
    ).sort_values("cp_rank_R")

    rank_summary_path = save_rank_sweep_summary_outputs(
        summary_df=rank_summary_df,
        fig_root_dir=FIG_ROOT_DIR,
    )
    ...
    global_summary = {
        "model_type": "CP_PARAFAC_MLP_RANK_SWEEP",
        "decomposition_set": "TRAIN",
        "cp_rank_list": [
            int(rank)
            for rank in CP_RANK_LIST
        ],
        "hardware": hardware_info,
        "regressor": REGRESSOR,
        "test_indices_one_based": [
        ...
        "rank_sweep_summary_csv": os.path.abspath(
            rank_summary_path
        ),
        ...
        "rank_summary_records": (
            rank_summary_df.to_dict(orient="records")
        ),
    }

    global_summary_path = os.path.join(
        FIG_ROOT_DIR,
        "cp_rank_sweep_global_summary.json",
    )
    ...
    # =================== Final Global Console Summary ===================

    print(
        "\n"
        + "=" * 24
        + " GLOBAL RANK SWEEP SUMMARY "
        + "=" * 24
    )
    ...
    print(
        f"All outputs in: "
        f"{os.path.abspath(FIG_ROOT_DIR)}"
    )
    ...
    print_columns = [
        ...
        "decomposition_time_seconds",
        "dnn_training_time_seconds",
        "dnn_inference_mean_ms",
        "field_reconstruction_mean_ms",
    ...
    print(
        rank_summary_df[print_columns].to_string(index=False)
    )

    print("\n--- Global Saved Files ---")
    print("cp_rank_sweep_summary.csv")
    print("cp_rank_sweep_global_summary.json")
    print("cp_dcr_rank_sweep.csv")
    print("cp_dcr_vs_rank.png")
    print("cp_rank_vs_accuracy.png")
    print("cp_rank_vs_offline_time.png")
    print("cp_rank_vs_online_time.png")
