...
class DataAndPODManager:
    def __init__(self, boundary_path, snapshot_dir):
        ...
        self.mean_field = None  # 将在这里保存平均场
    ...
    def load_data(self):
        ...
        # 【修正】对温度快照进行归一化 (方案A：按空间点进行缩放)
        # 先转置为 (n_conditions, n_points)
    ...
    def perform_pod(self, energy_threshold=0.9999):
        ...
        # 4. 计算模态系数 (形状 [n_conditions, r_space])
...
class PODTuckerModel:
    ...
    def train_network(self, epochs=300, patience=30):
        """训练神经网络: boundary_data -> condition_basis_coeffs"""
        print("\n训练神经网络以映射参数到工况基系数...")
...
def visualize_sparse_field(true_field, pred_field, coords, case_idx, save_dir="results_pod_tucker"):
    ...
    # 2. 选择一个Y平面进行切片 (可以适当加厚切片以包含更多点)
    y_slice_val = y_unique[len(y_unique) // 2]
    # 使用一个更厚的“切片”来获取足够的数据点
    ...
    # 4. 创建遮罩 (Mask)
    # isnan会告诉我们哪些点是无效的(在凸包外)
    mask = np.isnan(grid_true)

    # 将NaN值替换为一个中性值以便计算误差，但在绘图时我们会忽略它们
    grid_error = np.abs(np.nan_to_num(grid_true) - np.nan_to_num(grid_pred))
    # 应用遮罩到误差图
    grid_error[mask] = np.nan  # 让误差图也在无效区不显示

    # 5. 绘图
    fig, axes = plt.subplots(1, 3, figsize=(24, 7))
    fig.patch.set_facecolor('white')  # 设置画布背景为白色
    ...
    # 设置一个中性背景色
    for ax in axes:
        ax.set_facecolor('#EAEAF2')  # 设置绘图区背景为淡灰色
