...
# Regression options
REGRESSOR = "GPR"          # <<< 已改为 "SVR"。可选 "SVR" 或 "GPR"
GPR_KERNEL = "RBF"      # "RBF" or "Matern" (if REGRESSOR="GPR")

# SVR hyperparameters
SVR_KERNEL = "rbf"         # 'rbf' | 'linear' | 'poly' | 'sigmoid'
SVR_C = 10.0
SVR_EPSILON = 1e-3
SVR_GAMMA = "scale"        # 'scale' | 'auto' 或数值
SVR_CACHE_MB = 500
...
def choose_regressor(name: str, y_dim: int):
    """
    Returns a multi-output regressor for predicting rn coefficients.
    """
    name = name.upper()
    if name == "GPR":
...
def main():
    ...
    #TEST_IDX_ONE_BASED = [2, 9, 20, 21, 56, 68, 72, 76, 84]
    TEST_IDX_ONE_BASED = [1, 22, 26, 30, 35, 36, 38, 41, 48]
    ...
    summary = {
        ...
        "test_indices": [int(x) for x in idx_test.tolist()],                 # 0-based
        "test_indices_one_based": [int(x) for x in (idx_test + 1).tolist()], # 1-based (for traceability)
