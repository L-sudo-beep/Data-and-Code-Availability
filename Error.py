...
# 三种POD展开顺序
# yzx: X方向变化最快，主要保留X方向邻接
# xzy: Y方向变化最快，主要保留Y方向邻接
# xyz: Z方向变化最快，主要保留Z方向邻接
...
def visualize_results(true_temp, pred_temp, coords, case_index, save_dir):
    """
    当前默认用于 y=constant 截面，即绘制 X-Z 平面。
    如果 PLANE_TYPE='y'，则横轴为 X，纵轴为 Z。
    """
...
# ======================================
# Vectorization and Adjacency Functions
# ======================================
def build_ordered_grid_indices(Nx, Ny, Nz, order):
    """
    根据向量化展开顺序，生成一维向量中每个位置对应的原始三维网格索引。
...
def build_ordered_coords(x_vals, y_vals, z_vals, order):
    """
    根据向量化展开顺序，生成与POD快照矩阵行顺序严格对应的坐标数组。
    返回坐标始终为 [X, Y, Z]。
    """
    coord_map = {
        'x': x_vals,
...
def map_hotspot_indices_to_current_order(pre_selected_indices, original_coords, current_coords):
    """
    将原始索引对应的物理热点坐标映射到当前展开顺序下的索引。
    这样可以保证不同展开顺序比较的是同一批物理热点。
    """
...
# ======================================
# Reshape Flat Data to 3D Tensor
# ======================================
...
# ======================================
# Save adjacency stats for all orders
# ======================================
...
# ======================================
# Main Function for One POD Unfolding Order
# ======================================
