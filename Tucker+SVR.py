...
------------------------------------------------------------------
This version uses SVR (Support Vector Regression) to predict N-mode coefficients
instead of MLP, and outputs per-case MAE/RMSE (full 3D & Y-slice) to CSV.
...
ENERGY_THRESH = 0.99  # cumulative energy threshold per mode; only used if USE_ADAPTIVE_RANKS=True
FALLBACK_RANKS = (15, 15, 15, 20)  # used if adaptive fails

# Centering and scaling
CENTER_ALONG_N = True      # mean-remove across N before HOSVD
SCALE_INPUTS = True        # Standardize 14D parameters
SCALE_OUTPUT_COEFF = True  # Standardize Un for regression targets (inverse-transform before reconstruction)

# Regression options
REGRESSOR = "SVR"          # "SVR" or "GPR"
...
SVR_C = 10.0
SVR_EPSILON = 1e-3
SVR_GAMMA = "scale"        # 'scale' | 'auto' or numeric
SVR_CACHE_MB = 500
...
def choose_regressor(name: str, y_dim: int):
    ...
    if name == "GPR":
        ...
        raise ValueError("Unknown REGRESSOR (supported: 'SVR', 'GPR').")
    return model
...
def main():
    ...
    # basic checks
    if np.any(idx_test < 0) or np.any(idx_test >= N_CASES):
        raise ValueError(f"Test indices out of range after zero-basing: {idx_test.tolist()}")
    if len(np.unique(idx_test)) != len(idx_test):
        raise ValueError("Duplicate indices found in TEST_IDX_ONE_BASED.")
    if len(idx_test) != 9:
        warnings.warn(f"[WARN] Expected 9 test cases, got {len(idx_test)}.")
    ...
    # Scale outputs (fit on train only)
    ...
    print(f"=== Fit regressor: {REGRESSOR} ===")
    ...
    preds = np.stack(preds, axis=-1)   # (NX, NY, NZ, n_test)
    gts   = np.stack(gts, axis=-1)     # (NX, NY, NZ, n_test)
    ...
    print("\nPer-Case Metrics:")
    for case1b, m, r, ms, rs in zip(TEST_IDX_ONE_BASED, mae_list, rmse_list, mae_slice_list, rmse_slice_list):
        print(f"  Case {case1b:>2d} | Full3D: MAE={m:.4f}, RMSE={r:.4f} | "
              f"Y={ys[y_slice_index]:.3f}m: MAE={ms:.4f}, RMSE={rs:.4f}")

    # ---- Save per-case metrics CSV ----
    per_case_df = pd.DataFrame({
        "case_index_one_based": TEST_IDX_ONE_BASED,
        "case_index_zero_based": (np.array(TEST_IDX_ONE_BASED) - 1),
        "full3d_mae": mae_list,
        "full3d_rmse": rmse_list,
        f"yslice_y_{ys[y_slice_index]:.3f}_mae": mae_slice_list,
        f"yslice_y_{ys[y_slice_index]:.3f}_rmse": rmse_slice_list
    })
    csv_path = os.path.join(FIG_DIR, "tucker_svr_metrics_by_case.csv")
    per_case_df.to_csv(csv_path, index=False, encoding="utf-8-sig")
    print(f"\nSaved per-case metrics CSV to: {os.path.abspath(csv_path)}")

    # ---- Save summary JSON (kept) ----
    summary = {
        ...
        "svr_kernel": SVR_KERNEL if REGRESSOR.upper()=="SVR" else None,
        "svr_params": {
            "C": SVR_C, "epsilon": SVR_EPSILON, "gamma": SVR_GAMMA, "cache_mb": SVR_CACHE_MB
        } if REGRESSOR.upper()=="SVR" else None,
        "random_state": RANDOM_STATE,
        "test_indices_zero_based": [int(x) for x in (np.array(TEST_IDX_ONE_BASED)-1).tolist()],
        "test_indices_one_based": [int(x) for x in TEST_IDX_ONE_BASED],
    ...
    for k, case_idx in enumerate(np.array(TEST_IDX_ONE_BASED)-1):
