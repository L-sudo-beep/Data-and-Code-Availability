...
# -*- coding: utf-8 -*-
"""
POD + MLP with DCR analysis and computational-time benchmarking.

Core timing indicators:
...
Data loading, plotting, metric calculation, and file writing are excluded from
...
# Timing repetitions. Use the same values for POD, Tucker, and CP.
...
def count_mlp_parameters(input_dim, output_dim, hidden_layers):
    ...
    for h in hidden_layers:
        total += prev * h + h       # Linear
        total += 2 * h              # BatchNorm gamma and beta
        prev = h
...
def main():
    ...
    # Boundary conditions and fixed split
    ...
    # Predictions used for both timing and accuracy
    model.eval()
    with torch.no_grad():
