...
- 保存逐工况 MAE/RMSE、折线图、切片云图与 metrics_summary.json
...
ENERGY_THRESH = 0.99  # 可切换 0.95 / 0.99 / 0.999
...
def main():
    ...
    for j, case_idx in enumerate(idx_test):
        coeff = Y_pred[j]  # (rn,)
        # 用训练核心重构（注意 G_train 的形状为 (rx,ry,rz,rn)）
    ...
    plt.xlabel("Case")
    plt.ylabel("Error (°C)")
    plt.title("Tucker+MLP Per-case Error (TRAIN-only, per-mode ranks)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    # =================== Summary ===================
    # 汇报每模能量前若干奇异值，可帮助论文画“能量-秩曲线”
    def sv_info(S):
        e = (S**2); cum = np.cumsum(e) / np.sum(e)
        # 仅保存前 min(50, len(S)) 个点，避免过大
        k = min(50, len(S))
        return cum[:k].tolist()
